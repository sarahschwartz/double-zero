...
const transfer = {
  ...
  blockNumber: 6681664,
  transactionHash:
    '0x99e37da46fd7b2913cef0c7a4366dbdc7eea42c42ace71b5c2b276dea74461ba',
...
describe('Transfers:', () => {
  ...
  describe('Table:', () => {
    ...
    it('renders transaction hash column', () => {
      ...
      ).toEqual('0x99e37da46fd...61ba');
    });

    it('renders timestamp column', () => {
      expect(renderResult!.getByTestId(elements.timestamp).textContent).toBe(
        useTimeAgo(transfer.timestamp).value,
      );
    });

    it('renders type column', () => {
      expect(renderResult!.getByTestId(elements.transferType).textContent).toBe(
        transfer.type,
      );
    });
    ...
    it('renders from column', () => {
      expect(
        renderResult!.getByTestId(elements.fromAddress).textContent,
      ).toEqual('0x0a27CaAeaD5...706b');
      expect(
        renderResult!.getByTestId(elements.transferFromOrigin).textContent,
      ).toEqual('L1');
      expect(
        renderResult!.getByTestId(elements.transferFromOriginTablet)
          .textContent,
      ).toEqual('L1');
    });
    ...
    it('renders to column', () => {
      expect(renderResult!.getByTestId(elements.toAddress).textContent).toEqual(
        '0xB2bAbfBBB09...1347',
      );
      expect(
        renderResult!.getByTestId(elements.transferToOrigin).textContent,
      ).toEqual('L1');
      expect(
        renderResult!.getByTestId(elements.transferToOriginTablet).textContent,
      ).toEqual('L1');
    });

    it('renders from/to column for tablet', () => {
      expect(
        renderResult?.getByText('0x0a27CaAeaD5...706b', {
          selector: '.tablet-column a',
        }),
      );
      expect(
        renderResult?.getByText('0xB2bAbfBBB09...1347', {
          selector: '.tablet-column a',
        }),
      );
    });
    ...
    it('renders amount column', () => {
      expect(
        renderResult!.getByTestId(elements.tokenAmount).textContent,
      ).toEqual('129187024.261081');
      expect(
        renderResult!.getByTestId(elements.tokenAmountPrice).textContent,
      ).toEqual('$129.19M');
    });
  });
