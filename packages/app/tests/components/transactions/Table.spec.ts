...
describe('Transfers:', () => {
  ...
  describe('Table:', () => {
    ...
    it('renders from/to column for tablet', () => {
      ...
      expect(
        renderResult?.getByText('0x3355d...aaf4', {
          selector: '.tablet-column a',
        }),
      );
    });
    ...
    it('renders value column', () => {
      ...
      expect(
        renderResult!.getAllByTestId(elements.tokenAmountPrice)[0].textContent,
      ).toEqual('$0.02');
    });
    ...
    it('renders fee column', () => {
      ...
      expect(
        renderResult!.getAllByTestId(elements.tokenAmountPrice)[1].textContent,
      ).toEqual('$0.12');
    });
  });
  ...
  describe('Table with pagination:', () => {
    ...
    describe('when total > pageSize', () => {
      ...
      beforeEach(() => {
        mockTransactions = useTransactionsMock({
          data: computed(() =>
            Array.from({ length: 10 }).map(() => transaction),
          ),
          total: computed(() => 11),
        });
        renderResult = render(Table, {
