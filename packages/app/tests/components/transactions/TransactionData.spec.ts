...
describe('TransactionDataComponent', () => {
  ...
  it('renders decoded data by default', () => {
    ...
    const parameterRows = container.querySelectorAll(
      '.method-parameters-table tbody tr',
    );
  ...
  it.skip('renders binary decoded data when selected in the dropdown', async () => {
    ...
    await fireEvent.click(
      container.querySelector('.show-as-dropdown .toggle-button')!,
    );
    ...
    expect(
      container.querySelector('.encoded-data-container')?.textContent,
    ).toBe(
