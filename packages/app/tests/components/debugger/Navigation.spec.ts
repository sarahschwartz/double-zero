...
describe('Navigation:', () => {
  ...
  it('renders 0 as an active step out of total by default', async () => {
    ...
    expect(
      container.querySelector<HTMLInputElement>('.active-index')?.value,
    ).toEqual('0');
  ...
  it('triggers nav:goTo event when previous button clicked', async () => {
    ...
    await fireEvent.click(
      getByTestId('previous-instruction-navigation-button'),
    );
  ...
  it('renders Cmd+K hotkey label for MacOs', () => {
    ...
    expect(
      container
        .querySelector('.navigation-search-input-container')
  ...
  it('renders searchText', async () => {
    ...
    expect(
      getByTestId<HTMLInputElement>(testId.traceSearchInput).value,
    ).toEqual('Hello');
    unmount();
  });
  ...
  it('triggers nav:goTo event on active step input blur', async () => {
    ...
    await fireEvent.update(
      container.querySelector<HTMLInputElement>('.active-index')!,
      '5',
    );
  ...
  it('sets total length as a maxlength of active step input', async () => {
    ...
    expect(
      container
        .querySelector<HTMLInputElement>('.active-index')
