...
describe('Contract:', () => {
    ...
    const { container } = render(Contract, {
      ...
      global: {
        ...
        stubs: {
          ...
          TransfersTable: { template: '<div />' },
        },
      },
    });
    expect(
      container.querySelector('.breadcrumb-item-active')?.textContent,
    ).toBe('Contract 0x0cc7...dc1b');
    expect(
      container
  ...
  it.skip('renders contract name in the headline when contract is verified', () => {
    ...
    expect(
      container.querySelector('.title-container')?.textContent?.trim(),
    ).toBe('DARA2');
  });
});
