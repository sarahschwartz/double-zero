...
describe('DebuggerView:', () => {
  ...
  it('has correct title', async () => {
    expect(
      i18n.global.t(
        routes.find((e) => e.name === 'debugger')?.meta?.title as string,
      ),
    ).toBe('zkEVM Debugger');
  });
  ...
  it('expands a node when enter pressed', async () => {
    ...
    await fireEvent.keyPress(getByTestId('file-list-item-toggle'), {
      key: 'Enter',
    });
  ...
  it("opens full screen mode on 'full screen' icon click", async () => {
    ...
    await fireEvent.click(
      container.querySelector('.full-screen-container > button')!,
    );
  ...
  it('renders Cmd+S hotkey label for MacOs', () => {
    ...
    expect(
      container.querySelector('.full-screen-container > span')!.textContent,
    ).toBe('Cmd + S');
  ...
  it('resets searchText when file reuploaded', async () => {
    ...
    await fireEvent.update(
      container.querySelector('.navigation-search-input')!,
      'He',
    );
    ...
    const input = await waitFor(() =>
      container.querySelector<HTMLInputElement>('.navigation-search-input'),
    );
  ...
  it('shows child metadata block if exist', async () => {
    ...
    const mockNavigation = vi
      .spyOn(useTrace, 'useTraceNavigation')
      .mockReturnValue({
        activeLines: ref({}),
        getActiveLines: () => {
          return null;
    ...
    await nextTick();
    expect(
      container.querySelector('.parent-child-address .actual-string')
        ?.textContent,
    ).toBe('0x01');
    expect(container.querySelector('.parent-child-type')?.textContent).toBe(
      'Child',
    );
