<template>
  ...
  <div class="flex justify-center flex-1">
    <div class="flex flex-1 flex-col items-center max-w-xl">
      <div
        class="w-full rounded-lg shadow-md mt-8 bg-white flex-1 p-4 space-y-4"
      >
        <div
        ...
        <div
          class="mt-4 p-4 bg-gray-100 rounded-lg border border-gray-400"
          :class="{ 'opacity-50': !rpcToken }"
        >
          <p class="font-bold mb-2">RPC Token:</p>
          <p class="font-mono break-all bg-white p-2 rounded text-sm">
            {{ rpcToken || 'Not available' }}
          </p>
          <p class="font-bold mb-2 mt-4">RPC URL:</p>
          <p class="font-mono break-all bg-white p-2 rounded text-sm">
            {{ rpcUrl || 'Not available' }}
          </p>
          <button
